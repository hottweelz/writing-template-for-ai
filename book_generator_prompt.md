...
- `book_project_data.md` — extended project details (optional; use if present)
- `MEMORY.md` — any previously established facts
- `docs/genre-routing.md` — which agents and protocols apply to this book's genre (consult before choosing structure and quality gates)
...
- Production note: EPUB/PDF are generated by `scripts/build.sh` (pandoc) — see the README "Producing your book file" section
...
- EPUB and PDF generation note (`scripts/build.sh`, pandoc)
...
**For nonfiction** (self-help, business, how-to, essay), replace gates 1–2 with: (1) Chapter-promise check — does each chapter deliver what it set out to prove? (2) Evidence/example sufficiency — is each major claim supported? Keep gates 3–6 (prose-suppression applies at word level only; structured formatting such as headers and lists is allowed). See `docs/genre-routing.md`.
